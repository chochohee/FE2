  ...
  return (
    <section>
      <ResultContainer>
        <ResultHeader>
          <h2>퀴즈 결과</h2>
          <ScoreBadge>{result.score.toFixed(1)} 점</ScoreBadge>
        </ResultHeader>
        <ResultDetails>
          <ResultMessage>{getScoreMessage()}</ResultMessage>
          <StatsList>
            <StatItem>
              <span>전체 문제</span>
              <strong>{result.totalQuestions} 문제</strong>
            </StatItem>
            <StatItem>
              <span>맞힌 문제</span>
              <strong>{result.correctAnswers} 문제</strong>
            </StatItem>
            <StatItem>
              <span>틀린 문제</span>
        ...
        <RestartButton onClick={onRestart}>재도전하기</RestartButton>
      </ResultContainer>
    </section>
  );
}
